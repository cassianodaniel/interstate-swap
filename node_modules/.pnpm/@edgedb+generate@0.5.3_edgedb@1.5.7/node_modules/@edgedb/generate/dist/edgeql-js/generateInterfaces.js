...
const generateInterfaces = (params) => {
    ...
    const writeModuleExports = (module) => {
        ...
        if (module.isRoot && module.name === "default") {
            const typeRefs = [
                ...module.types.values(),
                ...[...module.nestedModules.values()].map((nestedMod) => nestedMod.fullInternalName),
            ];
            for (const typeRef of typeRefs) {
                plainTypesCode.writeln([
                    `import ${typeRef.slice(module.internalName.length + 1)} = ${typeRef};`,
                ]);
            }
            plainTypesCode.writeln([(0, builders_1.t) `export type {`]);
            plainTypesCode.increaseIndent();
            plainTypesCode.writeln([
                typeRefs
